...
1. rudder with 8 positions on each side. 
1. The boat has a lifesaver buoy to 
create "man in water" alerts and maneuvers. 
1. Wind power on sails computes both blowing wind and airflow effect over the sails as wings at
tight angles. 
...
1. When the wind changes sides from the back, sails perform a full side change (revolution) that should be done
with care. 
1. Rudder is also affecting the boat drag according to its position, so if set to 90 degrees, it will not cause a turn, but slow the boat.
1. The colors of sails reflect the direction of the wind blow side (green when positive, red when negative). 
...
2. Windows 64-bits: Unzip the installation file and run the **Sailing.exe** executable for a full-screen experience.
